    ...
    try {
      ...
      if (edgeDetectedImage != null) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => CroppedImagePage(edgeDetectedImage: edgeDetectedImage),
          ),
        );
      } else {
      ...
      print('Error performing edge detection: $e');
  ...
  Future<Uint8List?> _detectEdges(String imagePath) async {
    try {
      ...
      if (originalImage != null) {
        ...
        // For demonstration, let's apply a simple edge detection filter
        final edgeDetectedImage = img.sobel(originalImage);

        return Uint8List.fromList(img.encodePng(edgeDetectedImage));
      } else {
...
class CroppedImagePage extends StatelessWidget {
  final Uint8List edgeDetectedImage;

  const CroppedImagePage({Key? key, required this.edgeDetectedImage}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Edge Detected Image"),
      ),
      body: Center(
        child: Image.memory(edgeDetectedImage),
      ),
    );
  }
}
